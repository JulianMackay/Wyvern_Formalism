...
Require Import common.lhs_sel_bnd_reduce.lhs_sel_bnd_reduce.
  ...
  | [Hsub : subtype (t_sel_upp ?x ?L ?T) _ = true |- _] =>
    rewrite subtype_sel_upp_reduce
      with
        (x1:=x)(L1:=L)(T1':=T)
      in Hsub; [|auto]
  | [Hsub : subtype (t_sel_bnd ?x ?L) _ = true |- _] =>
    rewrite subtype_sel_bnd_reduce
      with
        (x1:=x)(L1:=L)
      in Hsub; [|auto]
...
Ltac sub_auto :=
  match goal with
  | [|- ?G ⊢ (sel ?x ?L) <; (sel ?x ?L)] => apply s_refl
  end.

Ltac s_upper :=
  match goal with
  | [H : _ ⊢ ?x ∋ (type ?L ⩽ _) |- _ ⊢ sel ?x ?L <; _] => try solve [eapply s_upper_1; eauto]
  | [H : _ ⊢ ?x ∋ (type ?L ≝ _) |- _ ⊢ sel ?x ?L <; _] => try solve [eapply s_upper_2; eauto]
  | [H : _ ⊢ ?x ∋ (type ?L ⪯ _) |- _ ⊢ sel ?x ?L <; _] => try solve [eapply s_upper_3; eauto]
  end.

Ltac s_lower :=
  match goal with
  | [H : _ ⊢ ?x ∋ (type ?L ⩾ ?t) |- _ ⊢ _ <; sel ?x ?L] => try solve [eapply s_lower_1 with (t2:=t); eauto]
  | [H : _ ⊢ ?x ∋ (type ?L ≝ ?t) |- _ ⊢ _ <; sel ?x ?L] => try solve [eapply s_lower_2 with (t2:=t); eauto]
  end.

Ltac s_ext :=
  match goal with
  | [H : _ ⊢ ?t ≤⦂⦂ ?t' |- _ ⊢ ?t <; _] => try solve [eapply s_extend; eauto]
  end.
...
Proof.
  apply is_material_mutind;
    ...
    | [ |- is_material ([_ /t _] (top str _ rts)) ] => simpl; apply m_rfn_top; eauto
    | [ |- is_material ([_ /t _] ((sel _ _) str _ rts)) ] => simpl; apply m_rfn_sel; eauto
...
| conf_no_bound: forall G x L, (forall σ, G ⊢ x ∋ σ ->
                                id σ <> L) ->
                          wf_var G x ->
                          conforms G (sel x L) (t_sel_bnd x L)
...
| conf_nom : forall G L t T, conforms G t T ->
                        conforms_s G (type L ⪯ t) (t_nom L T)
...
Proof.
  ...
  apply conf_no_bound;
    ...
    |inversion w; subst; apply wf_concrete; crush].
  apply equiv_ctx_has with (G':=G) in H3; auto.
  contradiction (n σ).
  apply equiv_ctx_x_sym; inversion H0; auto.
...
Lemma subtype_refl_sel :
  forall G x L T1 T2, conforms G (sel x L) T1 ->
                 conforms G (sel x L) T2 ->
                 subtype T1 T2 = true.
Proof.
  intros.
  ...
  (*sel_bnd*)
  rewrite subtype_sel_bnd_sel_bnd_reduce
    with
      (x1:=x)(L1:=L)
      (x2:=x)(L2:=L);
    auto;
  ...
  rewrite subtype_sel_bnd_sel_upp_reduce
    with
      (x1:=x)(L1:=L)
      (x2:=x)(L2:=L)(T2':=T);
    auto;
  ...
  rewrite subtype_sel_bnd_sel_low_reduce
    with
      (x1:=x)(L1:=L)
      (x2:=x)(L2:=L)(T2':=T);
    auto;
  ...
  rewrite subtype_sel_bnd_sel_equ_reduce
    with
      (x1:=x)(L1:=L)
      (x2:=x)(L2:=L)(T2':=T);
    auto;
  ...
  rewrite subtype_sel_bnd_sel_nom_reduce
    with
      (x1:=x)(L1:=L)
      (x2:=x)(L2:=L)(T2':=T);
    auto;
  ...
  rewrite subtype_sel_upp_sel_bnd_reduce
    with
      (x1:=x)(L1:=L)(T1':=T)
      (x2:=x)(L2:=L);
    auto;
  ...
  rewrite subtype_sel_low_sel_bnd_reduce
    with
      (x1:=x)(L1:=L)(T1':=T)
      (x2:=x)(L2:=L);
    auto;
  ...
  rewrite subtype_sel_equ_sel_bnd_reduce
    with
      (x1:=x)(L1:=L)(T1':=T)
      (x2:=x)(L2:=L);
    auto;
  ...
  rewrite subtype_sel_nom_sel_bnd_reduce
    with
      (x1:=x)(L1:=L)(T1':=T)
      (x2:=x)(L2:=L);
    auto;
...
Proof.
  ...
  inversion H0; subst;
    try (apply member_uniqueness
         ...
         inversion H3;
         subst).
  contradiction (H4 (type L ⩽ t)); auto.
...
Theorem sub_implies_subtype_mutind :
  (forall G t1 t2, G ⊢ t1 <; t2 ->
              ord G ->
              is_material t2 ->
              forall T1 T2, conforms G t1 T1 ->
                       conforms G t2 T2 ->
                       subtype T1 T2 = true) /\
  ...
  (forall G s1 s2, G ⊢ s1 <;; s2 ->
              ord G ->
              is_material_s s2 ->
              forall T1 T2, conforms_s G s1 T1 ->
                       conforms_s G s2 T2 ->
                       subtype T1 T2 = true) /\
  ...
  (forall G ss1 ss2, G ⊢ ss1 <;;; ss2 ->
                ord G ->
                is_material_ss ss2 ->
                forall T1 T2, conforms_ss G ss1 T1 ->
                         conforms_ss G ss2 T2 ->
                         subtype T1 T2 = true).
Proof.
  ...
  inversion H3; subst;
    try (apply member_uniqueness
         ...
         inversion H6;
         subst).
  contradiction (H7 (type l ⩾ t2)); auto.
  apply subtype_rhs_sel_low_reduce
    with
      (x:=x)(L:=l)(T2':=T); auto;
    intros;
    intro Hcontra;
    subst T1; inversion H2.
  ...
  inversion H2; subst;
    try (apply member_uniqueness
         ...
         inversion H6;
         subst).
  contradiction (H7 (type l ⩽ t1)); auto.
  rewrite subtype_sel_upp_reduce
    with (x1:=x)(L1:=l)(T1':=T);
    auto.
  destruct T2; auto;
  ...
  rewrite subtype_all_all_reduce
    ...
    auto.
  apply conforms_tree_uniqueness
    with
      (T1:=T0)
    in H6;
    auto;
    subst.
  rewrite eq_tree_refl; rewrite H; auto.
  apply ord_con;
    intros;
    eauto.
  eapply conforms_notin in H2; notin_auto; eauto.
  ...
  rewrite H; auto;
    [apply ord_con; eauto
    |mat_auto; eapply is_material_ss_subst; eauto
    |].
  assert (Hequiv : ([c_ (length Γ) /ss 0] ss2) ⊢ ((top str ss1 rts) :: Γ) equiv_ss ((top str ss1 rts) :: Γ));
    [apply ord_equiv_ss_refl|].
  apply ord_con; eauto.
  apply equiv_ctx_ss_strengthening
    with
      (t1:=top str ss1 rts)(G1':=Γ)(t2:=top str ss1 rts)(G2':=Γ)
    in Hequiv;
    auto;
    try solve [apply ord_con; eauto].
  apply equiv_ctx_ss_weakening
    with
      (t1:=top str ss2 rts)(t2:=top str ss1 rts)
    in Hequiv;
    intros;
    eauto.
  apply conforms_equiv_ctx_ss with (G:=top str ss2 rts :: Γ);
    eauto;
    apply ord_con; eauto.
  rewrite subst_closed_ss; mat_auto; eauto.
  rewrite subst_closed_ss; mat_auto; eauto.


  inversion H3; subst; auto;
    try solve [inversion H1; subst; shape_auto; crush].
  rewrite subtype_rfn_sel_rfn_sel_eq_reduce
    ...
    auto.
  rewrite H; auto;
    [apply ord_con; intros; eauto;
     eapply conforms_notin; eauto
    |repeat mat_auto; eauto
    |].
  assert (Hequiv : ([c_ (length Γ) /ss 0] ss2) ⊢ ((sel x L str ss1 rts) :: Γ) equiv_ss ((sel x L str ss1 rts) :: Γ));
    [apply ord_equiv_ss_refl|].
  apply ord_con; intros; eauto.
  eapply conforms_notin; eauto.
  apply equiv_ctx_ss_strengthening
    with
      (t1:=sel x L str ss1 rts)(G1':=Γ)(t2:=sel x L str ss1 rts)(G2':=Γ)
    in Hequiv;
    auto;
    try solve [apply ord_con; eauto].
  apply equiv_ctx_ss_weakening
    with
      (t1:=sel x L str ss2 rts)(t2:=sel x L str ss1 rts)
    in Hequiv;
    intros;
    eauto.
  apply conforms_equiv_ctx_ss with (G:=sel x L str ss2 rts :: Γ);
    eauto;
    apply ord_con;
    intros;
    eauto;
    eapply conforms_notin; eauto.
  rewrite subst_closed_ss; mat_auto; eauto.
  apply ord_con; intros; eauto; eapply conforms_notin; eauto.
  rewrite subst_closed_ss; mat_auto; eauto.

  inversion H3; subst; auto;
    try solve [inversion H1; subst; shape_auto; crush].
  inversion H1; subst; shape_auto; crush.
  inversion H1; subst; shape_auto; crush.
  inversion H1; subst; shape_auto; crush.
  ...
  contradiction (H9 (type L ⩽ t)); auto.
  
  apply member_uniqueness with (s1:=type L ⩽ t0) in H4; auto;
    inversion H4; subst.
  subtype_reduce;
    auto;
  ...
  apply member_uniqueness with (s1:=type L ⩽ t) in H8; auto;
    inversion H8; subst.
  ...
  apply extends_uniqueness with (t1:=t) in H8; auto; subst t1.
  ...
  inversion H4; subst.
  apply member_uniqueness with (s1:=type L ⩽ t1) in H12; auto;
    inversion H12; subst t1.
  apply H15 in H11. 
  ...
  apply subtype_lhs_sha_sel_reduce
    ...
    try solve [intros;
               ...
               subst T2;
               inversion H3];
    apply extends_uniqueness with (t1:=t) in H8;
    auto;
    subst t1;
    apply H; auto.
  ...
  (*s-nom-nom*)
  apply conforms_tree_uniqueness_s with (T1:=T2) in H1; auto; subst T2.
  inversion H2; subst.
  erewrite subtype_nom_nom_reduce; eauto.
  rewrite eq_label_refl, eq_tree_refl; auto.
  ...
  intros G t T Hconf;
    ...
    try solve [match goal with
               ...
               | [H : (subtype t_top (t_sel_nom _ _ _) = true) |- _] =>
                 rewrite subtype_lhs_top_reduce in H; inversion H
               | [H : (subtype t_top (t_sel_bnd _ _) = true) |- _] =>
                 rewrite subtype_lhs_top_reduce in H; inversion H
...
Lemma subtype_equivalence_sel_upp_lhs :
  forall G2 t2 T2, conforms G2 t2 T2 ->
              ...
              forall G1 x L t1 T1, G1 ⊢ x ∋ (type L ⩽ t1) ->
                              conforms G1 (sel x L) (t_sel_upp x L T1) ->
                              forall G, t2 ⊢ G2 equiv_t G ->
                                   (sel x L) ⊢ G1 equiv_t G ->
...
Proof.
  intros G t2 T2 Hconf;
    ...
    eq_auto;
    equiv_ctx_auto;
    repeat equiv_ctx_solve;
    eauto;
    s_upper;
    repeat equiv_ctx_solve;
    try solve [apply s_upper_1 with (t1:=t1);
               [|eauto];
               equiv_ctx_solve].

  apply s_lower_1 with (t2:=t);
    [equiv_ctx_solve|equiv_ctx_auto; eauto].
...
Lemma subtype_equivalence_sel_equ_lhs :
  forall G2 t2 T2, conforms G2 t2 T2 ->
              is_material t2 ->
              forall G1 x L t1 T1, G1 ⊢ x ∋ (type L ≝ t1) ->
                              conforms G1 (sel x L) (t_sel_equ x L T1) ->
                              forall G, t2 ⊢ G2 equiv_t G ->
                                   (sel x L) ⊢ G1 equiv_t G ->
                                   (forall t2' T2', conforms G2 t2' T2' ->
                                               t2' ⊢ G2 equiv_t G ->
...
Proof.
  intros G t2 T2 Hconf;
    ...
    s_upper;
    repeat equiv_ctx_solve;
    try solve [apply s_upper_2 with (t1:=t1);
               [|eauto];
               equiv_ctx_solve].
...
Lemma subtype_equivalence_sel_nom_lhs :
  forall G2 t2 T2, conforms G2 t2 T2 ->
              is_material t2 ->
              forall G1 x L t1 T1, G1 ⊢ x ∋ (type L ⪯ t1) ->
                              conforms G1 (sel x L) (t_sel_nom x L T1) ->
                              forall G, t2 ⊢ G2 equiv_t G ->
                                   (sel x L) ⊢ G1 equiv_t G ->
                                   (forall t2' T2', conforms G2 t2' T2' ->
                                               t2' ⊢ G2 equiv_t G ->
...
Proof.
  intros G t2 T2 Hconf;
    ...
    s_upper;
    repeat equiv_ctx_solve;
    try solve [apply s_upper_3 with (t1:=t1);
               [|eauto];
               equiv_ctx_solve].
...
Lemma subtype_equivalence_sel_low_lhs :
  forall G t2 T2, conforms G t2 T2 ->
             forall x L t1, G ⊢ x ∋ (type L ⩾ t1) ->
                       forall T1, conforms G (sel x L) (t_sel_low x L T1) ->
...
Proof.
  ...
  exists t, (merge ss d); auto.

  exists (sel v l), ss; auto.

  exists top, ss; auto.
  
Qed.

Lemma subtype_rfn_sel_lhs_bot_rhs :
  forall G t T, conforms G t T ->
           forall t' ss, t = (t' str ss rts) ->
                    subtype T t_bot = false.
Proof.
  intros G t T Hconf;
    induction Hconf;
    intros;
  ...
  inversion H; subst.
  apply flat_is_rfn in H9;
    destruct H9 as [t'' Htmp];
    destruct Htmp as [ss'' Htmp];
    subst t.
  rewrite subtype_rfn_sel_reduce
    with
      (x1:=x)(L1:=L)(Ts1:=Ts)(T':=T);
    auto.
  apply IHHconf with (t':=t'')(ss:=ss'');
    auto.
  ...
  apply flat_is_rfn in H9;
    ...
    destruct Htmp as [ss'' Htmp];
    subst t.
  rewrite subtype_sha_sel_reduce
    with
      (x1:=x)(L1:=L)(T1':=T)(ss1:=ss);
    auto.
  apply IHHconf with (t':=t'')(ss:=ss'');
    auto.
Qed.
                    
Lemma subtype_bot_rhs :
  forall G t T, conforms G t T ->
...
Ltac subtype_equiv :=
  match goal with 
  | [H : subtype t_top ?T = true, H' : conforms ?G ?t ?T  |- _] =>    
    eapply subtype_equivalence_top_lhs with (G:=G) in H;
    [|apply H'];
    auto
  | [H : subtype ?T t_bot = true, Hconf : conforms ?G ?t ?T |- ?G ⊢ ?t <; bot] =>
    eapply subtype_bot_rhs; eauto
  | [H : subtype (t_sel_upp ?x ?L ?T) _ = true |- _] =>
    rewrite subtype_sel_upp_reduce
      with
        (x1:=x)(L1:=L)(T1':=T)
      in H;
    auto
  | [H : subtype (t_sel_equ ?x ?L ?T) _ = true |- _] =>
    rewrite subtype_sel_equ_reduce
      with
        (x1:=x)(L1:=L)(T1':=T)
      in H;
    auto
  | [H : subtype (t_sel_nom ?x ?L ?T) _ = true |- _] =>
    rewrite subtype_sel_nom_reduce
      with
        (x1:=x)(L1:=L)(T1':=T)
      in H;
    auto
  | [H : subtype (t_sel_low ?x ?L ?T) _ = true |- _] =>
    rewrite subtype_sel_low_reduce
      with
        (x1:=x)(L1:=L)(T1':=T)
      in H;
    auto
  | [H : subtype (t_rfn_top ?Ts) _ = true |- _] =>
    rewrite subtype_rfn_top_reduce
      with
        (Ts1:=Ts)
      in H;
    auto
  | [H : subtype (t_rfn_sel ?x ?L ?Ts ?T) _ = true |- _] =>
    rewrite subtype_rfn_sel_reduce
      with
        (x1:=x)(L1:=L)(Ts1:=Ts)(T':=T)
      in H;
    auto
  | [H : subtype (t_sha_top ?x ?L ?ss) _ = true |- _] =>
    rewrite subtype_sha_top_reduce
      with
        (x1:=x)(L1:=L)(ss1:=ss)
      in H;
    auto
  | [H : subtype (t_sha_sel ?x ?L ?ss ?T) _ = true |- _] =>
    rewrite subtype_sha_sel_reduce
      with
        (x1:=x)(L1:=L)(ss1:=ss)(T1':=T)
      in H;
    auto
  | [H : subtype (t_all ?T1 ?T2) _ = true |- _] =>
    rewrite subtype_all_reduce
      with
        (Ta1:=T1)(Tb1:=T2)
      in H;
    auto
...
Lemma subtype_equivalence_rfn_top_lhs :
  forall G2 t2 T2, conforms G2 t2 T2 ->
              is_material t2 ->
              forall G1 ss1 Ts1, conforms G1 (top str ss1 rts) (t_rfn_top Ts1) ->
                            forall G, top str ss1 rts ⊢ G1 equiv_t G ->
                                 t2 ⊢ G2 equiv_t G ->
                                 (forall ss2 Ts2, conforms G2 (top str ss2 rts) (t_rfn_top Ts2) ->
                                             top str ss2 rts ⊢ G2 equiv_t G ->
                                             is_material (top str ss2 rts) ->
                                             subtype Ts1 Ts2 = true ->
                                             (top str ss1 rts ::G) ⊢ ([c_ (length G) /ss 0] ss1) <;;; ([c_ (length G) /ss 0] ss2)) ->
                                 subtype (t_rfn_top Ts1) T2 = true ->
                                 G ⊢ (top str ss1 rts) <; t2.
Proof.
  intros G t T Hconf;
    induction Hconf;
    intros;
...
Lemma subtype_equivalence_rfn_top_rhs :
  forall G1 t1 T1, conforms G1 t1 T1 ->
              forall G2 ss2 Ts2, conforms G2 (top str ss2 rts) (t_rfn_top Ts2) ->
                            is_material (top str ss2 rts) ->
                            forall G, t1 ⊢ G1 equiv_t G ->
                                 top str ss2 rts ⊢ G2 equiv_t G ->
                                 (forall ss1 Ts1, conforms G1 (top str ss1 rts) (t_rfn_top Ts1) ->
                                             top str ss1 rts ⊢ G1 equiv_t G ->
                                             subtype Ts1 Ts2 = true ->
                                             (top str ss1 rts ::G) ⊢ ([c_ (length G) /ss 0] ss1) <;;; ([c_ (length G) /ss 0] ss2)) ->
                                 subtype T1 (t_rfn_top Ts2) = true ->
                                 G ⊢ t1 <; (top str ss2 rts).
Proof.
...
Lemma subtype_equivalence_rfn_sel_lhs :
  forall G2 t2 T2, conforms G2 t2 T2 ->
              ...
              forall G1 x L ss1 t1 Ts1 T1, conforms G1 (sel x L str ss1 rts) (t_rfn_sel x L Ts1 T1) ->
                                      ...
                                      forall G, sel x L str ss1 rts ⊢ G1 equiv_t G ->
                                           t2 ⊢ G2 equiv_t G ->
                                           (forall ss2 Ts2 T2', conforms G2 (sel x L str ss2 rts) (t_rfn_sel x L Ts2 T2') ->
                                                           sel x L str ss2 rts ⊢ G2 equiv_t G ->
                                                           is_material (sel x L str ss2 rts) ->
                                                           subtype Ts1 Ts2 = true ->
                                                           (sel x L str ss1 rts ::G) ⊢ ([c_ (length G) /ss 0] ss1) <;;; ([c_ (length G) /ss 0] ss2)) ->
                                           (forall t2' T2', conforms G2 t2' T2' ->
                                                       t2' ⊢ G2 equiv_t G ->
                                                       is_material t2' ->
                                                       subtype T1 T2' = true ->
...
Lemma subtype_equivalence_sel_low_rhs :
  forall G1 t1 T1, conforms G1 t1 T1 ->
              forall G2 x L t2 T2, G2 ⊢ x ∋ (type L ⩾ t2) ->
                              is_material (sel x L) ->
                              forall G, t1 ⊢ G1 equiv_t G ->
                                   (sel x L) ⊢ G2 equiv_t G ->
                                   conforms G2 (sel x L) (t_sel_low x L T2) ->
...
Proof.
  intros G t1 T1 Hconf;
    ...
    bool_auto;
    eq_auto;
    try solve [apply s_lower_1 with (t2:=t2);
               eauto;
               equiv_ctx_auto;
               equiv_ctx_solve];
    auto.
...
Lemma subtype_equivalence_sel_equ_rhs :
  forall G1 t1 T1, conforms G1 t1 T1 ->
              forall G2 x L t2 T2, G2 ⊢ x ∋ (type L ≝ t2) ->
                              ...
                              forall G, t1 ⊢ G1 equiv_t G ->
                                   (sel x L) ⊢ G2 equiv_t G ->
                                   conforms G2 (sel x L) (t_sel_equ x L T2) ->
                                   (forall t1' T1', conforms G1 t1' T1' ->
                                               t1' ⊢ G1 equiv_t G ->
                                               subtype T1' T2 = true ->
                                               G ⊢ t1' <; t2) ->
...
Proof.
  intros G t1 T1 Hconf;
    ...
    bool_auto;
    eq_auto;
    try solve [apply s_lower_2 with (t2:=t2);
               eauto;
               equiv_ctx_auto;
               equiv_ctx_solve];
    auto.
...
Lemma subtype_equivalence_sha_top_lhs :
  forall G t2 T2, conforms G t2 T2 ->
             is_material t2 ->
             ord G ->
             forall x L ss, conforms G (sel x L str ss rts) (t_sha_top x L ss) ->
                       subtype (t_sha_top x L ss) T2 = true ->
                       G ⊢ (sel x L str ss rts) <; t2.
Proof.
  intros G t2 T2 Hconf;
    induction Hconf;
    intros;
...
Lemma subtype_equivalence_sha_sel_lhs :
  forall G2 t2 T2, conforms G2 t2 T2 ->
             ...
             forall G1 x L ss t1, G1 ⊢ (sel x L str ss rts) ≤⦂⦂ t1 ->
                             forall G, (sel x L str ss rts) ⊢ G1 equiv_t G ->
                                  t2 ⊢ G2 equiv_t G ->
...
Proof.
  ...
  eapply s_lower_1 with (t2:=t); eauto.
  equiv_ctx_auto.
  eapply equiv_ctx_has; eauto.
  eapply IHHconf;
    eauto.
  ...
  eapply s_lower_2 with (t2:=t); eauto.
  equiv_ctx_auto.
  eapply equiv_ctx_has; eauto.
  eapply IHHconf;
    eauto.
...
Lemma subtype_equivalence_all_lhs :
  forall G2 t2 T2, conforms G2 t2 T2 ->
             is_material t2 ->
             ord G2 ->
             forall G1 t t1 T T1, conforms G1 (all t ∙ t1) (t_all T T1) ->
                             forall G, (all t ∙ t1) ⊢ G1 equiv_t G ->
                                  t2 ⊢ G2 equiv_t G ->
                                  length G1 = length G ->
                                  length G2 = length G ->
                                  (forall t2' T2', conforms (t :: G2) ([c_ (length G2) /t 0] t2') T2' ->
                                              (length G2) notin_t t2' ->
                                              (all t ∙ t2') ⊢ G2 equiv_t G ->
                                              is_material t2' ->
                                              subtype T1 T2' = true ->
                                              (t::G) ⊢ ([c_ (length G) /t 0] t1) <; ([c_ (length G) /t 0] t2')) ->
                                  subtype (t_all T T1) T2 = true ->
                                  G ⊢ (all t ∙ t1) <; t2.
Proof.
  intros G t T Hconf;
    induction Hconf;
  ...
  apply s_lower_1 with (t2:=t); eauto.
  eapply equiv_ctx_has; eauto.
  eapply IHHconf; equiv_ctx_auto; eauto.

  apply s_lower_2 with (t2:=t); eauto.
  eapply equiv_ctx_has; eauto.
  eapply IHHconf; equiv_ctx_auto; eauto.

  apply eqb_tree_eq in H10;
    subst T0.
  inversion H4; subst.
  apply conforms_uniqueness
    with (G1:=G)(t1:=t)
    in H13;
    auto;
    [|crush];
...
Lemma subtype_equivalence_all_rhs :
  forall G1 t1 T1, conforms G1 t1 T1 ->
             forall G2 t t2 T T2, conforms G2 (all t ∙ t2) (t_all T T2) ->
                             is_material (all t ∙ t2) ->
                             forall G, t1 ⊢ G1 equiv_t G ->
                                  (all t ∙ t2) ⊢ G2 equiv_t G ->
                                  length G1 = length G ->
                                  length G2 = length G ->
                                  (forall t1' T1', conforms G1 (all t ∙ t1') (t_all T T1') ->
                                              (all t ∙ t1') ⊢ G1 equiv_t G ->
                                              subtype T1' T2 = true ->
                                              (t ::G) ⊢ ([c_ (length G) /t 0] t1') <; ([c_ (length G) /t 0] t2)) ->
                                  subtype T1 (t_all T T2) = true ->
                                  G ⊢ t1 <; (all t ∙ t2).
...
Lemma subtype_equivalence_sel_bnd_lhs :
  forall G t2 T2, conforms G t2 T2 ->
             forall x L,conforms G (sel x L) (t_sel_bnd x L) ->
                   subtype (t_sel_bnd x L) T2 = true ->
                   G ⊢ (sel x L) <; t2.
Proof.
  intros G t2 T2 Hconf;
    induction Hconf;
...
Lemma subtype_equivalence_sel_bnd_rhs :
  forall G t T, conforms G t T ->
           forall x L, subtype T (t_sel_bnd x L) = true ->
                  G ⊢ t <; (sel x L).
Proof.
  intros G t T Hconf;
    induction Hconf;
...
Lemma subtype_implies_sub_mutind :
  (forall G1 t1 T1, conforms G1 t1 T1 ->
               ord G1 ->
               forall G2 t2 T2, conforms G2 t2 T2 ->
                           ord G2 ->
...
Proof.
  apply conforms_mutind;
    ...
    intros;
    equiv_ctx_auto;
    try solve [mat_auto; shape_auto; contra_auto];
    try solve [eapply subtype_equivalence_sha_top_lhs;
               equiv_ctx_auto;
               equiv_ctx_solve;
               eauto;
               eapply conforms_equiv_ctx; eauto];
    try solve [inversion H1; subst;
               subtype_reduce;
               bool_auto;
               eq_auto;
               destruct H 
                 with (G2:=G2)(t2:=t0)(T2:=T0)(G0:=G0)
                 as [Ha Hb];
               equiv_ctx_auto;
               mat_auto;
               eauto].
  ...
  (*top*)
  (*>:*)
  eapply subtype_equivalence_top_lhs; eauto.
  eapply conforms_equiv_ctx; eauto.
  ...
  (*sel no bound*)
  (*<:*)
  eapply subtype_equivalence_sel_bnd_lhs;
    eauto;
    eapply conforms_equiv_ctx; eauto.
  
  (*:>*)
  eapply subtype_equivalence_sel_bnd_rhs;
    eauto;
    eapply conforms_equiv_ctx; eauto.
  ...
  eapply subtype_equivalence_sel_upp_lhs; eauto;
    edestruct H; intros; eauto;
      equiv_ctx_solve; equiv_ctx_auto; auto;
        edestruct H as [Ha Hb]; eauto.
  ...
  (*sel lower*)
  (*<:*)
  eapply subtype_equivalence_sel_low_lhs with (t1:=t);
    eauto;
    equiv_ctx_solve;
    eapply conforms_equiv_ctx; eauto.
  ...
  (*sel equals*)
  (*<:*)
  eapply subtype_equivalence_sel_equ_lhs; eauto;
    edestruct H; intros; eauto;
      equiv_ctx_solve; equiv_ctx_auto; auto;
        edestruct H as [Ha Hb]; eauto.
  ...
  (*sel nom*)
  (*<:*)
  eapply subtype_equivalence_sel_nom_lhs; eauto;
    edestruct H; intros; eauto;
      equiv_ctx_solve; equiv_ctx_auto; auto;
        edestruct H as [Ha Hb]; eauto.
  ...
  (*rfn top*)
  (*<:*)
  eapply subtype_equivalence_rfn_top_lhs;
    intros;
    eauto.
  destruct H
    with (G2:=(top str ss2 rts)::G2)
         (ss2:=[c_ (length G0) /ss 0]ss2)(T2:=Ts2)(G0:=(top str ss rts) :: G0)
    as [Ha Hb];
    try rewrite <- H7, H6 in *;
    simpl;
    eauto.
  apply ord_con; auto.
  inversion H4; subst; crush.
  apply ord_con; intros; auto.
  eapply conforms_notin; eauto.
  apply ord_con; intros; auto.
  eapply conforms_notin; eauto; crush.
  apply subst_equiv_ctx_ss.
  apply equiv_ctx_ss_weakening; auto.
  apply ec_conc1 with (t:=top str ss rts);
    [apply typing_cons
    |rewrite <- H6; apply typing_cons
    |apply equiv_ctx_t_weakening; auto].
  rewrite subst_closed_ss;
    [|mat_auto; auto].
  apply equiv_ctx_ss_weakening;
    equiv_ctx_auto;
    intros;
    auto.
  eapply conforms_notin in H4;
    eauto;
    notin_auto;
    auto.
  rewrite H6 in *;
  ...
  eapply subtype_equivalence_rfn_top_rhs;
    ...
    eauto.
  destruct H
    with (G0:=(top str ss1 rts)::G0)
         (G2:=(top str ss1 rts)::G2)
         (ss2:=[c_ (length G) /ss 0] ss1)
         (T2:=Ts1)
    as [Ha Hb];
    simpl;
    eauto.
  ...
  apply ord_con;
    ...
    eauto.
  eapply conforms_notin; eauto; crush.
  mat_auto; rewrite subst_closed_ss; auto.
  apply equiv_ctx_ss_weakening; auto.
  apply subst_equiv_ctx_ss; auto.
  apply equiv_ctx_ss_weakening; intros; equiv_ctx_auto; eauto.
  eapply conforms_notin in H4;
    notin_auto;
    eauto.
  apply ec_conc1 with (t:=top str ss1 rts); auto;
    [rewrite <- H6, H7 in *; apply typing_cons
    |rewrite <- H6; apply typing_cons
    |].
  apply equiv_ctx_t_weakening; intros; auto.
  eapply conforms_notin; eauto.
  rewrite H6 in *.
  apply Hb; mat_auto; auto; eapply is_material_ss_subst; eauto.
  
  (*rfn sel*)
  (*<:*)
  apply subtype_equivalence_rfn_sel_lhs
    with (G2:=G2)(T2:=T2)(G1:=G)(t1:=t)(Ts1:=Ts)(T1:=T);
    intros;
    eauto.
  destruct H
    with (G2:=(sel x L str ss2 rts)::G2)
         (ss2:=[c_ (length G0) /ss 0] ss2)
         (T2:=Ts2)(G0:=(sel x L str ss rts)::G0)
    as [Ha Hb];
    eauto;
    try solve [crush].
  apply ord_con;
    ...
    auto.
  eapply conforms_notin; eauto.
  inversion H5; subst; rewrite H8; auto.
  apply ord_con;
    intros;
    auto.
  eapply conforms_notin; eauto.
  apply ord_con;
    ...
    auto.
  eapply conforms_notin; eauto; crush.
  apply subst_equiv_ctx_ss; eauto.
  apply equiv_ctx_ss_weakening; auto.
  apply ec_conc1 with (t:=sel x L str ss rts);
    [apply typing_cons
    |rewrite <- H7; apply typing_cons
    |apply equiv_ctx_t_weakening; intros; eauto].
  eapply conforms_notin; eauto.
  rewrite subst_closed_ss; mat_auto; eauto.
  apply equiv_ctx_ss_weakening; equiv_ctx_auto; intros; eauto.
  eapply conforms_notin in H5; notin_auto; eauto.
  rewrite H7 in *; apply Ha; repeat mat_auto; eauto.

  edestruct H0 as [Ha Hb]; eauto.
  eapply extend_equiv_ctx; eauto.

  (*>:*)
  apply subtype_equivalence_rfn_sel_rhs
    with (G1:=G2)(T1:=T2)(G2:=G)(Ts2:=Ts)(T2:=T);
    equiv_ctx_auto;
    intros;
    eauto.
  destruct H
    with (G2:=sel x L str ss1 rts :: G2)(ss2:=[c_ (length G2)/ss 0]ss1)(T2:=Ts1)(G0:=sel x L str ss1 rts :: G0)
    as [Ha Hb];
    simpl;
    eauto.
  apply ord_con;
  ...
  eapply conforms_notin; eauto; crush.
  rewrite subst_closed_ss; mat_auto; eauto.
  apply equiv_ctx_ss_weakening; eauto.
  apply subst_equiv_ctx_ss; eauto.
  apply equiv_ctx_ss_weakening; intros; equiv_ctx_auto; eauto.
  eapply conforms_notin in H5; notin_auto; eauto.
  apply ec_conc1 with (t:=sel x L str ss1 rts); eauto;
    try solve [try apply typing_cons;
               try rewrite <- H8;
               apply typing_cons].
  apply equiv_ctx_t_weakening; intros; eauto.
  eapply conforms_notin; eauto.
  try rewrite <- H8, <- H7 in *.
  apply Hb; repeat mat_auto; eauto.
  ...
  (*all*)
  (*<:*)
  apply subtype_equivalence_all_lhs
    with (G2:=G2)(T2:=T2)(G1:=G)(T:=T)(T1:=T');
    intros;
    eauto.
  destruct H0
    with (G2:=t::G2)(G0:=t::G0)(T2:=T2')
         (t2:=[c_ (length G0) /t 0]t2')
    as [Ha Hb];
    simpl;
    eauto;
    equiv_ctx_auto;
    try solve [apply ord_con;
               ...
               eauto;
               eapply conforms_notin; eauto; crush].
  inversion H5; subst; rewrite H8; eauto.
  apply subst_equiv_ctx; eauto;
    [apply equiv_ctx_t_weakening; eauto
    |apply ec_conc1 with (t:=t); eauto;
     try solve [try apply typing_cons;
                rewrite <- H7;
                apply typing_cons]];
    apply equiv_ctx_t_weakening;
  ...
  apply subst_equiv_ctx; eauto.
  apply equiv_ctx_t_weakening; intros; eauto.
  destruct (le_lt_dec (S (length G2)) n0) as [Hle|Hlt].
  apply conforms_notin with (n:=n0) in H5; eauto; crush.
  eapply subst_notin in H5; eauto.
  assert (n0 = length G2); [crush|subst; auto].
  apply ec_conc1 with (t:=t); eauto;
    [rewrite H8
    |
    |];
  ...
  (*>:*)
  apply subtype_equivalence_all_rhs
    with (G1:=G2)(T1:=T2)(G2:=G)(T:=T)(T2:=T');
    intros;
    equiv_ctx_auto;
    eauto.
  destruct H0
    with (G2:=t::G2)(T2:=T1')(G0:=t::G0)
         (t2:=[c_ (length G0) /t 0]t1')
    as [Ha Hb];
    simpl;
    try solve [apply ord_con;
               intros;
  ...
  apply equiv_ctx_t_weakening; intros; eauto.
  eapply conforms_notin in H5; notin_auto; eauto.
  eapply ec_conc1 with (t:=t);
    try solve [apply typing_cons];
    try solve [rewrite H8;
               apply typing_cons].
  eapply equiv_ctx_t_weakening; intros; eauto.
  eapply conforms_notin in H5; notin_auto; eauto.
  rewrite <- H7 in *; apply Hb; repeat mat_auto; eauto.
  ...
  destruct H 
    ...
    mat_auto;
    eauto.
  apply beq_tree_eq in H10; subst T0.
  eapply conforms_uniqueness with (t1:=t0) in c;
    eauto;
    [subst t0; auto|crush].
  ...
  inversion H2;
    ...
    subtype_reduce;
    bool_auto.
  edestruct H as [Ha Hb], H0; eauto;
    repeat equiv_ctx_auto;
    repeat mat_auto;
    eauto.
                ...
                forall G2 ss2 T2, conforms_ss G2 ss2 T2 ->
                             ...
                             forall G, ord G ->
                                  ...
                                  (is_material_ss ss1 ->
                                   ...
                                   G ⊢ ss2 <;;; ss1)).
Proof.
  destruct subtype_implies_sub_mutind; crush.
Qed.

Lemma subtype_implies_sub_type :
  (forall G1 t1 T1, conforms G1 t1 T1 ->
               ord G1 ->
               forall G2 t2 T2, conforms G2 t2 T2 ->
                           ord G2 ->
...
Lemma subtype_implies_sub_decl :
  (forall G1 s1 T1, conforms_s G1 s1 T1 ->
               ord G1 ->
               forall G2 s2 T2, conforms_s G2 s2 T2 ->
                           ord G2 ->
...
Lemma subtype_implies_sub_decls :
  (forall G1 ss1 T1, conforms_ss G1 ss1 T1 ->
                ord G1 ->
                forall G2 ss2 T2, conforms_ss G2 ss2 T2 ->
                             ord G2 ->
...
Lemma subtype_implies_sub_type_actual :
  forall T1 T2, subtype T1 T2 = true ->
           forall G t1 t2, conforms G t1 T1 ->
                      conforms G t2 T2 ->
                      ord G ->
                      is_material t2 ->
                      G ⊢ t1 <; t2.
Proof.
  intros;
    eapply subtype_implies_sub_type;
    eauto;
    apply ord_equiv_refl; auto.
Qed.

Lemma subtype_implies_sub_decl_actual :
  forall D1 D2, subtype D1 D2 = true ->
           forall G σ1 σ2, conforms_s G σ1 D1 ->
                      ord G ->
                      conforms_s G σ2 D2 ->
                      is_material_s σ2 ->
                      G ⊢ σ1 <;; σ2.
Proof.
  intros;
    eapply subtype_implies_sub_decl;
    eauto;
    apply ord_equiv_s_refl; auto.
Qed.

Lemma subtype_implies_sub_decls_actual :
  forall Ds1 Ds2, subtype Ds1 Ds2 = true ->
             forall G σs1 σs2, conforms_ss G σs1 Ds1 ->
                          ord G ->
                          conforms_ss G σs2 Ds2 ->
                          is_material_ss σs2 ->
                          G ⊢ σs1 <;;; σs2.
Proof.
  intros;
    eapply subtype_implies_sub_decls;
    eauto;
    apply ord_equiv_ss_refl; auto.
Qed.

Theorem sub_implies_subtype_type_actual :
  (forall G t1 t2, G ⊢ t1 <; t2 ->
              ord G ->
              is_material t2 ->
              forall T1 T2, conforms G t1 T1 ->
                       ...
                       subtype T1 T2 = true).
Proof.
  destruct sub_implies_subtype_mutind; crush.
Qed.

Theorem sub_implies_subtype_decl_actual :
  (forall G s1 s2, G ⊢ s1 <;; s2 ->
              ord G ->
              is_material_s s2 ->
              forall T1 T2, conforms_s G s1 T1 ->
...
Theorem sub_implies_subtype_decls_actual :
  (forall G ss1 ss2, G ⊢ ss1 <;;; ss2 ->
                ord G ->
                is_material_ss ss2 ->
                forall T1 T2, conforms_ss G ss1 T1 ->
