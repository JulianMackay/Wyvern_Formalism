...
Lemma subtype_lhs_rfn_sel_reduce :
  forall T1 x1 L1 Ts1 T1', T1 = t_rfn_sel x1 L1 Ts1 T1' ->
                      forall T2, subtype T1' T2 = true ->
                            (T2 <> t_nil) ->
(*                            (forall x2 L2 Ts2 T2', T2 = t_rfn_sel x2 L2 Ts2 T2' ->
                                              (eq_var x1 x2 = false) \/
                                              (eq_label L1 L2 = false)) ->*)
...
Proof.
  ...
  subst T1.
  rewrite subtype_rfn_sel_reduce with (x1:=x1)(L1:=L1)(T':=T1')(Ts1:=Ts1);
    [|auto].
